...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        long LSN = logManager.appendToLog(new CommitTransactionLogRecord(transNum, entry.lastLSN));
...
//        return -1L;
    ...
    public long abort(long transNum) {
        // TODO(proj5): implement
//        System.out.println("ADSvadv");
        TransactionTableEntry entry = transactionTable.get(transNum);
        long LSN = logManager.appendToLog(new AbortTransactionLogRecord(transNum, entry.lastLSN));
        ...
        if (entry.transaction.getStatus() == Transaction.Status.ABORTING) {
            ...
            while (record.getPrevLSN().isPresent()) {
                if (record.isUndoable()) {
                    // undo
                    Pair<LogRecord, Boolean> undo = record.undo(entry.lastLSN);
                    Boolean flush = undo.getSecond();
                    // emit
                    long LSN = logManager.appendToLog(undo.getFirst());
                    undo.getFirst().redo(diskSpaceManager, bufferManager);
                    if (flush) logManager.flushToLSN(LSN);
                    if (record.getPageNum().isPresent()) {
                        dirtyPageTable.putIfAbsent(record.getPageNum().get(), LSN);
                    }
                    entry.lastLSN = LSN;
                }
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        if (before.length > (BufferManager.EFFECTIVE_PAGE_SIZE / 2)) {
            LogRecord undoRecord = new UpdatePageLogRecord(transNum, pageNum, entry.lastLSN, pageOffset, before, null);
            entry.lastLSN = logManager.appendToLog(new UpdatePageLogRecord(transNum, pageNum, logManager.appendToLog(undoRecord), pageOffset, null, after));
            dirtyPageTable.putIfAbsent(pageNum,entry.lastLSN);
        } else {
            entry.lastLSN = logManager.appendToLog(new UpdatePageLogRecord(transNum, pageNum, entry.lastLSN, pageOffset, before, after));
            dirtyPageTable.putIfAbsent(pageNum,entry.lastLSN);
        }
...
//        return -1L;
    ...
    public void rollbackToSavepoint(long transNum, String name) {
        ...
        while (record.getPrevLSN().isPresent() && record.getPrevLSN().get() >= LSN) {
            long lsn = record.getPrevLSN().get();
            if (record.getUndoNextLSN().isPresent() && record.getUndoNextLSN().get() <= LSN) break;
            Pair<LogRecord, Boolean> p = record.undo(transactionEntry.lastLSN);
            Boolean flush = p.getSecond();
            long CLRLSN = logManager.appendToLog(p.getFirst());
            if (flush) logManager.flushToLSN(CLRLSN);
            p.getFirst().redo(diskSpaceManager, bufferManager);
            if (lsn == LSN) break;
            if (record.isUndoable()) transactionEntry.lastLSN = CLRLSN;
...
//              *   - update the transaction table
                ...
                TransactionTableEntry entry = transactionTable.get(transNum);
                entry.lastLSN = Math.max(Record.LSN,entry.lastLSN);
//              *   - if it's page-related (as opposed to partition-related),
                if (Record.getPageNum().isPresent()) {
//                  *   - add to touchedPages
...
//                  *   - update DPT (alloc/free/undoalloc/undofree always flushes changes to disk)
                    if (Record.type.equals(LogType.UPDATE_PAGE) || Record.type.equals(LogType.UNDO_UPDATE_PAGE)) {
                        dirtyPageTable.putIfAbsent(Record.getPageNum().get(), Record.LSN);
                    } else {
            ...
            // * If the log record is a begin_checkpoint record:
            // * - Update the transaction counter
            if (Record.getMaxTransactionNum().isPresent() && Record.type.equals(LogType.BEGIN_CHECKPOINT)) {
                updateTransactionCounter.accept(Record.getMaxTransactionNum().get());
            }
            if (Record.getTransNum().isPresent()
                    && (Record.type.equals(LogType.COMMIT_TRANSACTION )|| Record.type.equals(LogType.ABORT_TRANSACTION) || Record.type.equals(LogType.END_TRANSACTION))) {
                long transNum = Record.getTransNum().get();
                if (!transactionTable.containsKey(transNum)) {
                    startTransaction(newTransaction.apply(transNum));
                }
                ...
                if (Record.type.equals(LogType.COMMIT_TRANSACTION)) {
                    entry.transaction.setStatus(Transaction.Status.COMMITTING);
                }
                if (Record.type.equals(LogType.ABORT_TRANSACTION)) {
                    entry.transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);
                }
                if (Record.type.equals(LogType.END_TRANSACTION)) {
...
//            If the log record is an end_checkpoint record:
            if (Record.type.equals(LogType.END_CHECKPOINT)) {
//                Copy all entries of checkpoint DPT (replace existing entries if any)
                Map<Long, Long> dirtyPages = Record.getDirtyPageTable();
                Map<Long, Pair<Transaction.Status, Long>> transTable = Record.getTransactionTable();
                Map<Long, List<Long>> touchedPages = Record.getTransactionTouchedPages();
//              dpt
                for (Long transNum: dirtyPages.keySet()) {
                    dirtyPageTable.put(transNum, dirtyPages.get(transNum));
                }
//              transTable
                for (Long transNum : transTable.keySet()) {
                    Pair<Transaction.Status, Long> p = transTable.get(transNum);
                    Transaction.Status status = p.getFirst();
                    Long lsn = p.getSecond();
//                      * - Update lastLSN to be the larger of the existing entry's (if any) and the checkpoint's;
//                      *   add to transaction table if not already present.
                    if (transactionTable.containsKey(transNum)) {
                        TransactionTableEntry entry = transactionTable.get(transNum);
                        ...
                        entry.lastLSN = Math.max(lsn, entry.lastLSN);
                    } else {
                        startTransaction(newTransaction.apply(transNum));
                        TransactionTableEntry entry = transactionTable.get(transNum);
                        if (entry.transaction.getStatus() == Transaction.Status.COMPLETE) {
                            ...
                            entry.transaction.setStatus(status);
                            entry.lastLSN = lsn;
                        }
                    }
                }
//              touchedpages
                for (Long transNum : touchedPages.keySet()) {
                    List<Long> list = touchedPages.get(transNum);
                    transactionTable.get(transNum).touchedPages.addAll(list);
                    if (transactionTable.get(transNum).transaction.getStatus() != Transaction.Status.COMPLETE) {
                        for (Long n : list) {
                            acquireTransactionLock(transactionTable.get(transNum).transaction, getPageLockContext(n), LockType.X);
                        }
                    }
                }
            }
        ...
        for (TransactionTableEntry entry : transactionTable.values()) {
            ...
            if (status.equals(Transaction.Status.COMMITTING)) {
                entry.transaction.cleanup();
                entry.transaction.setStatus(Transaction.Status.COMPLETE);
                entry.lastLSN = logManager.appendToLog(new EndTransactionLogRecord(transNum, entry.lastLSN));
                transactionTable.remove(transNum);
            }
    ...
    void restartRedo() {
        ...
        // finding the smallest lsn in dpt
        ...
        while (records.hasNext()) {
            ...
            if (record.isRedoable()) {
                // page related
                if (record.getPageNum().isPresent()) {
                    Page page = bufferManager.fetchPage(getPageLockContext(record.getPageNum().get()).parentContext(), record.getPageNum().get(), false);
                    try {
...
//        PairFirstReverseComparator comparator = ;
        ...
        for (TransactionTableEntry entry : transactionTable.values()) {
            ...
            if (entry.transaction.getStatus() == Transaction.Status.RECOVERY_ABORTING) {
                pq.add(new Pair<>(lastLSN, transNum));
            }
        }
        while (!pq.isEmpty()) {
            Pair<Long, Long> p = pq.remove();
            Long first = p.getFirst();
            Long second = p.getSecond();

            LogRecord record = logManager.fetchLogRecord(first);
            if (record.getTransNum().isPresent() && record.isUndoable()) {
                TransactionTableEntry entry = transactionTable.get(record.getTransNum().get());

                // if the record is undoable, undo it
                Pair<LogRecord, Boolean> undo = record.undo(entry.lastLSN);
                // emit the appropriate CLR
                Boolean flush = undo.getSecond();
                entry.lastLSN = logManager.appendToLog(undo.getFirst());
                if (flush) logManager.flushToLSN(entry.lastLSN);
                undo.getFirst().redo(diskSpaceManager, bufferManager);

                // update dpt

                if (record.getPageNum().isPresent() && record.type == LogType.UPDATE_PAGE) {
                    long pageNum = record.getPageNum().get();
                    if (dirtyPageTable.containsKey(pageNum) && dirtyPageTable.get(pageNum).equals(entry.lastLSN)) {
                        dirtyPageTable.remove(pageNum);
                    }
                    dirtyPageTable.put(pageNum, entry.lastLSN);
                }
            }
            // replace the entry in the set should be replaced with a new one, using the undoNextLSN (or prevLSN if none) of the record; and
            // if the new LSN is 0, end the transaction and remove it from the queue and transaction table.
            Long LSN;
            if (record.getPrevLSN().isPresent() && record.getUndoNextLSN().isPresent()) {
                LSN = record.getUndoNextLSN().get();
            } else {
                LSN = record.getPrevLSN().get();
            }
            if (LSN == 0L) {
                end(second);
                transactionTable.remove(second);
            } else {
                pq.add(new Pair<>(LSN, second));
            }
        }
    }
