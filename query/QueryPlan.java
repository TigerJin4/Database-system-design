...
//        System.out.println(startTableName);
        QueryOperator lowerOp = this.minCostSingleAccess(this.startTableName);
        map.put(table, lowerOp);
        ...
        while (tableIter.hasNext()) {
            table = new HashSet<>();
            String s = tableIter.next();
            table.add(s);
//            System.out.println(s);
            lowerOp = this.minCostSingleAccess(s);
            map.put(table, lowerOp);
        }
        ...
        for (int i=0; i < this.joinTableNames.size(); i++) {
            prevMap = this.minCostJoins(prevMap, map);
        }
...
//        QueryOperator minOp = null;
        ...
        for (int index : getEligibleIndexColumns(table)) {
            QueryOperator operator = new IndexScanOperator(transaction, table, selectColumnNames.get(index), this.selectOperators.get(index), this.selectDataBoxes.get(index));
            if (operator.estimateIOCost() < minOp.estimateIOCost()) {
                minOp = operator;
                minCol = index;
            }
        }
        QueryOperator result = addEligibleSelections(minOp, minCol);
...
//        return map;
        for (Set set : prevMap.keySet()) {
            ...
            for (int i = 0; i < joinTableNames.size(); i++) {
                //get the left side and the right side (table name and column)
                String[] left = getJoinLeftColumnNameByIndex(i), right = getJoinRightColumnNameByIndex(i);

                String leftName = left[0], rightName = right[0], leftCol = left[1], rightCol = right[1];

                QueryOperator out;

                Set otherKey;

                QueryOperator thisOp = prevMap.get(set);
//              Case 1. Set contains left table but not right, use pass1Map to
//              fetch the right operator to access the rightTable
                if (set.contains(leftName) && !set.contains(rightName)) {
                    otherKey = getKey(rightName, pass1Map);
                    QueryOperator otherOp = pass1Map.get(otherKey);
                    out = minCostJoinType(thisOp, otherOp, leftCol, rightCol);
                }
//              Case 2. Set contains right table but not left, use pass1Map to
//              fetch the right operator to access the leftTable.
                else if (set.contains(rightName) && !set.contains(leftName)) {
                    otherKey = getKey(leftName, pass1Map);
                    QueryOperator otherOp = pass1Map.get(otherKey);
                    out = minCostJoinType(otherOp, thisOp, leftCol, rightCol);
                }
//              Case 3. Set contains neither or both the left table or right table (continue loop)
                else {
                    continue;
...
//                outTables.addAll(set);
                outSet.addAll((Set<String>) set);
                outSet.addAll((Set<String>) otherKey);
                int prevCost, cost;
                if (prevMap.containsKey(outSet)) {
                    prevCost = prevMap.get(outSet).estimateIOCost();
                } else {
                    prevCost = Integer.MAX_VALUE;
                }
                if (map.containsKey(outSet)) {
                    cost = map.get(outSet).estimateIOCost();
                } else {
                    cost = Integer.MAX_VALUE;
                }
                if (out.estimateIOCost() < Math.min(cost, prevCost)) {
                    map.put(outSet, out);
                }
            }
        }
    ...
    /**
     * Helper to fetch the key from pass1Map with matching string k
     * @return the key from pass1Map with matching string k
     */
    private Set getKey(String k, Map<Set, QueryOperator> pass1Map) {
        Set<Set> set = pass1Map.keySet();
        ...
        Optional<Set> r = newset.stream().findFirst();
        assert r.isPresent() : "Key not found " + k;
        return r.get();
    }
