...
public class LockManager {
    ...
    private class ResourceEntry {
        ...
        boolean checkCompatible(LockType lockType, long except) {
            ...
            for(Lock lock : locks) {
                if(!lock.transactionNum.equals(except) && !LockType.compatible(lock.lockType, lockType)) {
                    return false;
                }
            }
            return true;
        ...
        void grantOrUpdateLock(Lock lock) {
            ...
            if (transactionLocks.get(lock.transactionNum)== null){
                transactionLocks.put(lock.transactionNum, new ArrayList<>(Arrays.asList(lock)));
                locks.add(lock);
            } else {
                // update locks
                for (Lock l: locks){
                    if (lock.transactionNum.equals(l.transactionNum)){
...
//            return;
        ...
        private void processQueue() {
            // TODO(proj4_part1): implement
            for (Lock lock : locks){
                while (!waitingQueue.isEmpty() && checkCompatible(waitingQueue.peekFirst().lock.lockType, lock.transactionNum)){
                    Lock lockToAdd =waitingQueue.getFirst().lock;
                    locks.add(lockToAdd);
                    transactionLocks.get(lockToAdd.transactionNum).add(lockToAdd);
                }
            }
        }
    ...
    private List<Lock> getLockList(long num) {
        transactionLocks.putIfAbsent(num, new ArrayList<>());
        return transactionLocks.get(num);
    }
    ...
    throws DuplicateLockRequestException, NoLockHeldException {
        ...
        synchronized (this) {
            ...
            if (!r.waitingQueue.isEmpty()) {
                r.addToQueue(request,false);
                transaction.prepareBlock();
                shouldblock = true;
            } else if (!r.locks.isEmpty() && !r.checkCompatible(lockType, transNum)){
                r.addToQueue(request,true);
            ...
            for (ResourceName n : releaseLocks) {
                if (r.waitingQueue.isEmpty() || n.equals(name)) {
    ...
    public void acquire(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            LockRequest request =  new LockRequest(transaction, lock);
            ResourceEntry r = getResourceEntry(name);
            if (!r.waitingQueue.isEmpty()||!r.checkCompatible(lockType,transaction.getTransNum())) {
                r.waitingQueue.addLast(request);
                transaction.prepareBlock();
                shouldBlock = true;

            } else if (r.locks.isEmpty()||r.checkCompatible(lockType,transaction.getTransNum())){
                r.locks.add(lock);
                List<Lock> locks = getLocks(transaction);
                locks.add(lock);
                transactionLocks.put(transaction.getTransNum(),locks);
            } else {
                getResourceEntry(name).waitingQueue.addLast(request);
                transaction.prepareBlock();
                shouldBlock = true;
            }
        }
    ...
    throws NoLockHeldException {
        ...
        synchronized (this) {
            ...
            for (Lock lock : getLocks(name)) {
                if (transaction.getTransNum() == lock.transactionNum) {
                    r.releaseLock(lock);
                    break;
                }
            }
            if (!r.waitingQueue.isEmpty()) {
                LockRequest request = r.waitingQueue.getFirst();
                r.locks.add(request.lock);
            }
            while (!r.waitingQueue.isEmpty()) {
                LockRequest request = r.waitingQueue.getFirst();
                long transNum = request.transaction.getTransNum();
                List<Lock> ls  = getLocks(name);
                for (Lock lock : ls) {
...
//                        break;
                ...
                if (!r.checkCompatible(request.lock.lockType, transaction.getTransNum())){
                    break;
                }
                r.locks.add(request.lock);
                Lock locktoadd = new Lock(name, request.lock.lockType, transNum);
                if (transactionLocks.get(transNum) == null) {
                    transactionLocks.put(transNum, new ArrayList<>(Arrays.asList(locktoadd)));
                } else {
                    for (Lock lock : getLocks(name)) {
                        if (lock.name.equals(request.lock.name)) {
                            transactionLocks.get(transNum).remove(lock);
                            break;
                        }
                    }
                    transactionLocks.get(transNum).add(locktoadd);
                }
                request.transaction.unblock();
                getResourceEntry(name).waitingQueue.removeFirst();
            }
        }
    }
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        ...
        synchronized (this) {
            ResourceEntry r = getResourceEntry(name);
            long transNum = transaction.getTransNum();
            List<Lock> locks = getLockList(transNum);
            Lock oldLock = null;
            if (locks.isEmpty()) {
                throw new NoLockHeldException("No lock is held at the moment");
            }
            ...
            if (oldLock == null) {
                throw new NoLockHeldException("No same named lock held on a resource");
            }
            Lock newLock = new Lock(name, newLockType, transNum);
            if (r.checkCompatible(newLockType, transNum)) {
                locks.remove(oldLock);
                locks.add(newLock);
                r.locks.remove(oldLock);
                r.locks.add(newLock);
                return;
            }
            else {
                r.addToQueue(new LockRequest(transaction, newLock, Arrays.asList(oldLock)), true);
                transaction.prepareBlock();
//                shouldblock = true;
            }
        }
