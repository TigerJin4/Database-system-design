    ...
    throws InvalidLockException, DuplicateLockRequestException {
        ...
        if (parent != null) {
            if (!LockType.canBeParentLock(parent.getExplicitLockType(transaction), lockType)){
                throw new InvalidLockException("Not a child type");
            } else {
                lockman.acquire(transaction, getResourceName(), lockType);
                int counter = parent.numChildLocks.getOrDefault(transaction.getTransNum(), 0);
                parent.numChildLocks.put(transaction.getTransNum(), counter + 1);
            }
        } else {
            lockman.acquire(transaction, getResourceName(), lockType);
        }
...
//                Integer newCount = parent.numChildLocks.get(transaction.getTransNum()) != null ? parent.numChildLocks.get(transaction.getTransNum()) + 1 : 1;
//                parent.numChildLocks.put(transaction.getTransNum(), newCount);
//            }
//        } else {
//            lockman.acquire(transaction, name, lockType);
//        }
    ...
    throws NoLockHeldException, InvalidLockException {
        ...
        if (numChildLocks != null && numChildLocks.containsKey(transNum)) {
            throw new InvalidLockException("Invalid Lock");
        }
        if (parent != null && (parent.numChildLocks.containsKey(transNum))) {
...
//            throw new UnsupportedOperationException("Context is readonly.");
//        }
//        Integer num = numChildLocks.get(transaction.getTransNum());
//        if(num == null || num == 0) {
//            lockman.release(transaction, name);
//            if(parent != null) {
//                Integer newCount = parent.numChildLocks.get(transaction.getTransNum()) - 1;
//                parent.numChildLocks.put(transaction.getTransNum(), newCount);
//            }
//        } else {
...
//        }
     ...
     boolean SIX(LockType lockType){
        return (lockType == LockType.S || lockType == LockType.IS || lockType == LockType.SIX);
     }
    boolean upgradeSIX(LockType lockType) {
        return (lockType==LockType.S || lockType==LockType.IX || lockType==LockType.IS);
    }
...
//        ResourceName name = getResourceName();
        LockType oldType = getExplicitLockType(transaction);
        if (readonly) {
            throw new UnsupportedOperationException("This transaction is read only");
        }
        if (oldType.equals(newLockType)) {
            throw new DuplicateLockRequestException("Duplicated lock type");
        }
        ...
        // SIX case
        List<ResourceName> torelease = new ArrayList<>(Arrays.asList(this.name));
        for (Lock l : lockman.getLocks(transaction)) {
            if (SIX(l.lockType)) {
                torelease.add(l.name);
            }
        }
        if (newLockType.equals(LockType.SIX) && upgradeSIX(oldType)){
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        List<Lock> childLocks = new ArrayList<>();
        ...
        if (!indicator){
            throw new NoLockHeldException("No lock with the same name");
        }
        if(!childLocks.isEmpty()) {
            List<Lock> length = new ArrayList<>();
            for(Lock childLock : childLocks) {
                if (SIX(childLock.lockType)) {
                    length.add(childLock);
                }
            }
            numChildLocks.put(transNum, 0);
            if(length.size() == childLocks.size()) {
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        // TODO(proj4_part2): implement
        if(getExplicitLockType(transaction) != LockType.NL && getExplicitLockType(transaction) != LockType.IX || parent == null) {
            ...
            if(parentType == LockType.S || parentType == LockType.X) {
                return parentType;
            } else if(parentType == LockType.SIX) {
                return LockType.S;
            }
        }
        return getExplicitLockType(transaction);
