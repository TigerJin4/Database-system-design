...
//        return Optional.empty();
        ...
        BPlusNode childNode = this.getChild(numLessThanEqual(key, keys));
        Optional<Pair<DataBox, Long>> optional = childNode.put(key, rid);
        if (optional.isPresent()) {
            Pair<DataBox, Long> pair = optional.get();
            DataBox newKey = pair.getFirst();
            long newPageNum = pair.getSecond();
            // putting the key in
            int index = numLessThanEqual(newKey, keys);
            ...
            if (keys.size() < 2*d+1) {
                ...
                // split from overflow
                List<DataBox> KeyList = new ArrayList<>();
                List<Long> ChildrenList = new ArrayList<>();

                // move info to the right node
                int n = (int) Math.ceil(keys.size()/2) + 1;
                for (int i = n; i < keys.size(); i++) {
                    KeyList.add(keys.get(i));
                    ChildrenList.add(children.get(i));
                }
                ChildrenList.add(children.get(keys.size()));
                for (int i = n+1; i < children.size(); i++) {
                    children.remove(i);
                    keys.remove(i - 1);
                }
                ...
                // push up
                InnerNode rightNode = new InnerNode(metadata, bufferManager, KeyList, ChildrenList, treeContext);
                sync();
                optional = Optional.of(new Pair<>(keys.remove(d), rightNode.getPage().getPageNum()));
            }
        }
        sync();
        return optional;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() < 2*d+1) {
            ...
            Optional<Pair<DataBox, Long>> optional = rightMostChild.bulkLoad(data, fillFactor);
            if (optional.isPresent()) {
                Pair<DataBox, Long> pair = optional.get();
        ...
        if (keys.size() > 2*d) {
            ...
            InnerNode rightInnerNode = new InnerNode(metadata, bufferManager,  KeyList, ChildrenList,  treeContext);
            Pair<DataBox, Long> result = new Pair<>(keys.remove(d),  rightInnerNode.getPage().getPageNum());
            sync();
