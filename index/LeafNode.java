...
//        sync();
        ...
        while (d < keys.size()) {
            KeyList.add(keys.get(d));
            RidList.add(rids.get(d));
            keys.remove(d);
            rids.remove(d);
        }
        // after split
        LeafNode rightNode = new LeafNode(metadata, bufferManager, KeyList, RidList, rightSibling, treeContext);
        //update rightsibling
        rightSibling = Optional.of(rightNode.getPage().getPageNum());
        Pair<DataBox, Long> pair = new Pair<>(rightNode.keys.get(0), rightNode.getPage().getPageNum());
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        int loadLimit = (int) Math.ceil(2*d*fillFactor);
        // below the load limit for each leaf node
        while (data.hasNext() && (keys.size()<=loadLimit)) {
            Pair<DataBox, RecordId> next = data.next();
            DataBox first = next.getFirst();
            if (keys.contains(first)) {
                sync();
                throw new BPlusTreeException("Duplicate key");
            }
            int index = InnerNode.numLessThanEqual(first, keys);
            keys.add(index, first);
            rids.add(index, next.getSecond());
        }
        if (keys.size() > loadLimit) {
            List<DataBox> KeyList = new ArrayList<>();
            List<RecordId> RidList = new ArrayList<>();
            KeyList.add(keys.get(keys.size() - 1));
            RidList.add(rids.get(rids.size() - 1));
            ...
            rightSibling = Optional.of(rightNode.getPage().getPageNum());
            sync();
            return Optional.of(new Pair<>(KeyList.get(0), rightNode.getPage().getPageNum()));
        }
        sync();
        return Optional.empty();
...
//        return;
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        if (siblingPageId == -1) {
            ...
            rightSibling = Optional.of(siblingPageId);
        }
        int n = buf.getInt();
        for (int i = 0; i < n; ++i) {
